...
import { getRandomUserToMentionInGuild } from '../../../utils/InteractionUtils';
import { getUserPreferredName } from '../../core/CoreUtils';
...
async function formatResponse(client: Bot, message: Message, response: string) {
    ...
    return response
        .replace(
            /\{sender}/gi,
            await getUserPreferredName(message.author.id, guild)
        )
        .replace(
            /\{random}/gi,
            await getRandomUserToMentionInGuild(guild, [
                message.author.id,
                client.user!.id,
            ]).then((member) => getUserPreferredName(member.id, guild))
        );
}
...
async function sendQuirkyResponse(client: Bot, message: Message) {
    ...
    if (
        !REPLY_EXCLUDED_CHANNELS.includes(message.channelId) &&
        Math.random() < REPLY_TO_MESSAGE_CHANCE
    ) {
        await message.reply({
