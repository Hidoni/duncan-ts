...
import { clearName, insertOrUpdateName } from '../NameQueries';
...
async function handleNameSetSubCommand(
    ...
    if (!name) {
        ...
        await insertOrUpdateName(interaction.user.id, name);
        await getSafeReplyFunction(
            client,
            interaction
        )({
        ...
        client.logger?.info(
            `Set ${interaction.user.username}'s name to ${name}`
        );
    }
}
...
async function handleNameClearSubCommand(
    ...
    await clearName(interaction.user.id);
    await getSafeReplyFunction(
        client,
        interaction
    )({
