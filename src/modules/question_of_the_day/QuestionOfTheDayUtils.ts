...
export async function changeQuestion(client: Bot) {
            ...
            .then(async (messages) => {
                if (messages.size === MAX_PINS_IN_CHANNEL) {
                    const oldMessage = messages
                        ...
                        .find(
                            (message) =>
                                message.content.match(QOTD_MESSAGE_REGEX) !=
                                null
                        );
                    if (oldMessage) {
                        client.logger?.debug(
                            `Unpinning message with id ${oldMessage.id}`
                        );
                        await oldMessage.unpin();
                    } else {
                        client.logger?.warn(
                            `Found no messages to unpin in QotD channel`
                        );
                    }
                }
                await msg.pin();
    ...
    await getUnusedQuestions().then((questions) => {
