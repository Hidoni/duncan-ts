...
import { getBrowniePoints } from '../../brownie_points/BrowniePointsQueries';
...
export class InteractCommand {
    ...
    protected async resolveBrowniePoints(
        ...
        for (const match of response.matchAll(BROWNIE_POINT_REGEX)) {
            ...
            await getBrowniePoints(interaction.user.id).then((points) => {
                points.points += delta;
                points.save();
            });
    ...
    protected async resolveMentions(
        ...
        response = response.replace(
            ...
            await getUserPreferredName(interaction.user.id, interaction.guild)
        );
        ...
        for (const match of response.matchAll(RANDOM_MENTION_REGEX)) {
            ...
            response = response.replace(
                match[0],
                await getUserPreferredName(randomUser.id, interaction.guild)
            );
        }
        return response;
    }
