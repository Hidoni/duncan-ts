...
import { FibbageQuestionState } from '../models/FibbageQuestion';
import { ModalHandlerFunction } from '../../../interfaces/ModalHandler';
import { isFibbageOnBreak } from '../FibbageUtils';
...
export const handler: ModalHandlerFunction = async (client, interaction) => {
    ...
    const question = await getFibbageQuestion(Number.parseInt(questionId));
    if (!question) {
        client.logger?.error(
    ...
    await question.save();
    await insertFibbageAnswer(answer, interaction.user.id, true, question);
    await getFibbageStats(interaction.user.id).then(async (stats) => {
