...
import {
    ...
    FibbagePrompt,
    FibbagePrompts,
} from './interfaces/FibbagePrompts';
import { FibbageScoreSummary } from './interfaces/FibbageScoreSummary';
import {
    getAllCustomFibbagePrompts,
    getAllFibbageQuestions,
    getFibbageStats,
    getQuestionsInState,
    insertFibbageAnswer,
    insertFibbageQuestion,
} from './FibbageQueries';
...
function convertCustomFibbagePromptsToRegularOnes(
    custom: FibbageCustomPrompt[]
): FibbagePrompts {
    ...
    for (const prompt of custom) {
        ...
        if (!customFibbagePromptHasRightAmountOfAnswers(answers)) {
            throw new Error(
                `Unexpected amount of default answers (${answers.length}) for custom prompt ${prompt.id}`
            );
        }
        regularPrompts[prompt.question] = {
...
async function sendButtonPromptToUser(
    ...
    const dbQuestion = await insertFibbageQuestion(question, user.id);
    await user.send({
...
export async function remindUsersToAnswerQuestions(client: Bot) {
    const userQuestions = await getQuestionsInState(FibbageQuestionState.ASKED);
...
export async function promptUserWithQuestion(
    ...
    const unaskedQuestions = await getUnaskedQuestionsForUser(
        client,
        askedQuestions,
        user
    );
...
export async function promptUsersWithQuestions(client: Bot) {
    ...
    const askedQuestions = await getAllFibbageQuestions();
...
export async function promptUsersForFibs(client: Bot) {
    ...
    const questionsThatNeedFibs = await getQuestionsInState(
...
async function fillMissingAnswersForQuestion(
    ...
    for (let i = 0; i < amountOfAnswersToAdd; i++) {
        ...
        await insertFibbageAnswer(answer, client.user!.id, false, question);
    }
}
...
export async function postNewQuestions(client: Bot) {
    let questions = await getQuestionsInState(FibbageQuestionState.PROMPTED, {
        loadAnswers: true,
    });
    ...
    for (const answerGroup of answerGroups) {
        if (answerGroup[0].isCorrect) {
            for (const answer of answerGroup) {
                ...
                const authorStats = await getFibbageStats(answer.user);
                for (const guess of answer.guesses) {
                    const guesserStats = await getFibbageStats(guess.user);
                    client.logger?.debug(
...
export async function showResultsForQuestions(client: Bot) {
    const questions = await getQuestionsInState(FibbageQuestionState.IN_USE, {
        loadAnswers: true,
        loadGuesses: true,
    });
    if (questions.length === 0) {
